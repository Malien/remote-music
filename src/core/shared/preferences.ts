...
export class Preferences {
    ...
    public static get currentVersion(): string {
        if (!this._version) {
            ...
            return this._version as string
    ...
    public constructor({ client, server }: PrefConstructorArgs = {}){
        ...
        this.version = Preferences.currentVersion
    }
...
export function merge(data: any): Preferences{
    ...
    if (versionComparator.compare(version, Preferences.currentVersion) == 0) {
...
export function read(path: string): Preferences {
    let data = fs.readFileSync(path).toString()
    let parsedData = JSON.parse(data)
    if (parsedData.version != Preferences.currentVersion) {
